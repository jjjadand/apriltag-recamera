...
double fx = (3.15 / 0.00139); // pixel size:1.39um x 1.39um, f = 
double fy = (3.15 / 0.00139);
double cx = (1920 / 2);
double cy = (1080 / 2);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < zarray_size(detections); i++) 
    {
        ...
        //inital inferce
        estimate_pose_for_tag_homography(tag_info, pose_inferce);

        //advance inferce, iteration 50?
        // double* err1; 
        // double* err2;
        // apriltag_pose_t* pose1; 
        // apriltag_pose_t* pose2;
        // int iter = 80;
        // estimate_tag_pose_orthogonal_iteration(tag_info, err1, pose1, err2, pose2, iter);
        
        //more easier to advance inferce
        estimate_tag_pose(tag_info, pose_inferce);
        ...
        for(int j = 0;j < 9;j++)
        {
            ...
            printf("%ld  ",pose_inferce->R->data[i]);
        }
        
        printf("\nTranslation matrix :\n");
        for(int j = 0;i < j;i++)
        {
            printf("%ld  ",pose_inferce->t->data[i]);
        }
